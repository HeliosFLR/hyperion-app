		...
		<div class="grid w-full grid-cols-1 gap-8 md:grid-cols-4" data-testid="stats-panel">
			...
			<div class="space-y-4">
				...
				<div class="space-y-2">
					<div class="flex items-baseline gap-2">
						<div class="text-sm text-gray-300">hySOL:</div>
						<div class="font-mono text-3xl font-bold text-white">
							~{Number(formatEther(stats.hySOLApy)).toFixed(4)}%
						</div>
					</div>
					<div class="flex items-baseline gap-2">
						<div class="text-sm text-gray-300">hyJitoSOL:</div>
						<div class="font-mono text-3xl font-bold text-white">
							~{Number(formatEther(stats.hyJitoSOLApy)).toFixed(4)}%
						</div>
					</div>
				</div>
			</div>
			...
			<div class="space-y-4">
				...
				<div class="space-y-1 font-mono text-sm text-gray-400">
					<div>hySOL: {Number(formatEther(stats.totalEligibleCymSOL)).toFixed(2)}</div>
					<div>hyJitoSOL: {Number(formatEther(stats.totalEligibleCyJitoSOL)).toFixed(2)}</div>
				</div>
			</div>

			<!-- Monthly Rewards -->
			<div class="space-y-4">
				<div class="text-sm text-gray-300">Monthly HYPR Rewards</div>
				<div class="font-mono text-3xl font-bold text-white">
					Total: {Number(formatEther(TOTAL_REWARD)).toLocaleString()}
				</div>
				<div class="space-y-1 font-mono text-sm text-gray-400">
					<div>hySOL: {Number(formatEther(stats.rewardsPools.cySOL)).toLocaleString()}</div>
					<div>hyJitoSOL: {Number(formatEther(stats.rewardsPools.cyWeth)).toLocaleString()}</div>
				</div>
			</div>
		</div>
	{/if}
