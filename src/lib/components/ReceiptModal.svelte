<script lang="ts">
	...
	const checkBalance = async () => {
		...
		if (!readableAmountToRedeem || readableAmountToRedeem === '') {
			amountToRedeem = BigInt(0);
			solToReceive = BigInt(0);
			return;
		}

		try {
			isCalculating = true;
			const _solToReceive = await readContract($wagmiConfig, {
				abi: erc20PriceOracleReceiptVaultAbi,
				functionName: 'previewRedeem',
				address: $selectedCyToken.address,
				args: [amountToRedeem, receipt.tokenId]
			});
			solToReceive = _solToReceive as bigint;
		} catch {
			solToReceive = BigInt(0);
			amountToRedeem = BigInt(0);
		} finally {
	...
	$: maxRedeemable =
		($balancesStore.balances[receipt.token || 'hySOL']?.signerBalance ?? 0n) <
		(erc1155balance ?? 0n)
			? $balancesStore.balances[receipt.token || 'hySOL']?.signerBalance ?? 0n
			: erc1155balance ?? 0n;
	...
	$: insufficientcySOL =
		($balancesStore.balances[receipt.token || 'hySOL']?.signerBalance ?? 0n) < amountToRedeem;
...
>
	<div class="flex w-full flex-col justify-between font-semibold text-white sm:flex-row">
		...
		<div class="flex flex-row gap-4">
			{#key readableBalance}{#if readableBalance}
					<span in:fade={{ duration: 700 }}>{formatEther(receipt.totalSOL ?? 0n)}</span>
				{/if}{/key}
		</div>
	</div>
	<div class="flex w-full flex-col justify-between font-semibold text-white sm:flex-row">
	...
	>
		...
		<div class="flex flex-col">
			...
			/>
			<p class="my-2 text-left text-xs font-light sm:text-right" data-testid="sol-balance">
	...
	<div class="hidden w-full flex-col items-center justify-center font-semibold text-white sm:flex">
		...
		<div class="flex flex-row items-center gap-2 overflow-ellipsis">
			<span class="flex overflow-ellipsis" data-testid="sol-to-receive">
				{formatEther(solToReceive)}
	...
	>
		<div class="flex w-full flex-col items-center justify-center gap-1 text-right">
			...
			<div class="flex flex-row items-center gap-2 overflow-ellipsis">
				<span class="flex overflow-ellipsis" data-testid="sol-to-receive-mobile">
					{formatEther(solToReceive)}
					{token.underlyingSymbol}
				</span>
			</div>
