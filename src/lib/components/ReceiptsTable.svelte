	...
	const mappedReceipts = receipts.map((receipt) => {
		...
		if (!receipt.balance || !receipt.tokenId) {
			return {
				...receipt,
				totalSOL: BigInt(0),
				readableSOLPerReceipt: '0.00000',
				readableTotalSOL: '0.00000'
			};
		}

		const balance = BigInt(receipt.balance);
		const tokenId = BigInt(receipt.tokenId);
		const totalSOL = (balance * BigInt(10 ** 18)) / tokenId;
		const solPerReceipt = BigInt(10 ** 36) / tokenId;

		return {
			...receipt,
			totalSOL: totalSOL,
			readableSOLPerReceipt: Number(formatEther(solPerReceipt)).toFixed(5),
			readableTotalSOL: Number(formatEther(totalSOL)).toFixed(5)
		};
	});
</script>
