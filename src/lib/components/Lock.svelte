<script lang="ts">
	...
	enum ButtonStatus {
		INSUFFICIENT_mSOL = 'INSUFFICIENT mSOL',
		READY = 'LOCK'
	}
	...
	<div class="flex w-full flex-col items-center justify-center gap-10" data-testid="lock-container">
		...
		>
			<div class="flex flex-col gap-0">
				<span>hy{$selectedCyToken.underlyingSymbol} PER {$selectedCyToken.underlyingSymbol}</span>
				<a
					href={base + '/docs/why-solana'}
					class="cursor-pointer text-xs font-light hover:underline"
					target="_blank"
					data-testid="price-ratio-link">How does Hyperion use the Pyth?</a
				>
			</div>
		...
		<div class="hidden w-full flex-col gap-2 sm:flex">
			...
			>
				{#key $balancesStore.stats[$selectedCyToken.name].lockPrice}
					<span data-testid="calculated-hysol"
			...
			<div
				class="flex w-full items-center justify-center gap-2 text-center text-lg font-semibold text-white sm:text-xl"
			>
				<span class="text-sm" data-testid="calculated-hysol-usd"
		...
		<div class="flex w-full flex-col items-center gap-2 sm:hidden">
			...
			>
				{#key $balancesStore.stats[$selectedCyToken.name].lockPrice}
					<span data-testid="calculated-hysol-mobile"
						>{!amountToLock ? '0' : formatEther($balancesStore.swapQuotes.cyTokenOutput)}</span
					>
				{/key}
				<span>{$selectedCyToken.name}</span>
			...
			<div
				class="flex w-full items-center justify-center gap-2 text-center text-lg font-semibold text-white sm:text-xl"
			>
				<span class="text-sm" data-testid="calculated-hysol-usd-mobile"
					>Current market value ~$ {amountToLock
						? formatUnits($balancesStore.swapQuotes.cusdxOutput, 6)
						: '0'}</span
				>
	...
	<div class="p-1 text-center sm:p-4">
		...
		>
			<li class="relative pl-2">
				...
				This front end is a tool for interacting with the Hyperion smart contracts.
			</li>
			<li class="relative pl-2">
				<span class="absolute -left-4">•</span>
			...
			<li class="relative pl-2">
				<span class="absolute -left-4">•</span>
				Smart contracts are immutable and cannot be upgraded or modified. Hyperion has been audited, however
			...
			<li class="relative pl-2">
				<span class="absolute -left-4">•</span>
				Hyperion relies on oracles to determine the ${$selectedCyToken.underlyingSymbol}/USD price.
				These are maintained by Pyth Network.
			</li>
			<li class="relative pl-2">
				<span class="absolute -left-4">•</span>
