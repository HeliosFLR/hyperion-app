...
describe('Unlock Component', () => {
	...
	it('should display hySOL balance correctly when wallet is connected', async () => {
		mockConnectedStore.mockSetSubscribeValue(true);
		mockSignerAddressStore.mockSetSubscribeValue('mockWalletAddress');
		render(Unlock);

		await waitFor(() => {
			const balanceText = screen.getByTestId('hysol-balance');
	...
	it('should display receipts table when receipts are available', async () => {
		...
		await waitFor(() => {
			expect(screen.queryByText('NO hySOL RECEIPTS FOUND...')).not.toBeInTheDocument();
	...
	it('should show "NO RECEIPTS FOUND" message when no receipts are available', async () => {
		...
		await waitFor(() => {
			expect(screen.getByText('NO hySOL RECEIPTS FOUND...')).toBeInTheDocument();
		});
	});
	...
	it('should display correct token name when a different token is selected', async () => {
		...
		const newTokenButton = screen.getByTestId('hyJitoSOL-button');
		await userEvent.click(newTokenButton);

		await waitFor(() => {
			expect(screen.queryByText('NO hyJitoSOL RECEIPTS FOUND...')).toBeInTheDocument();
		});
	});
});
