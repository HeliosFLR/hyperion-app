		...
		{:else}
			<div class="space-y-2">
				<div class="grid grid-cols-6 gap-8 text-sm text-gray-300">
					<div>Account</div>
					<div>Net hySOL</div>
					<div>hySOL rewards (HYPR)</div>
					<div>Net hyJitoSOL</div>
					<div>hyJitoSOL rewards (HYPR)</div>
					<div>Total Estimated HYPR</div>
				</div>
				{#if leaderboard?.length > 0}
					{#each leaderboard as entry, i}
						...
						>
							...
							<div class=" font-medium text-white">
								{(+formatEther(entry.eligibleBalances.hySOL)).toFixed(4)}
							</div>
							<div class="flex flex-col gap-y-2 font-medium text-white">
								<span>{(+formatEther(entry.shares.hySOL.rewardsAmount)).toFixed(4)}</span>
								<span>
									({(+formatUnits(entry.shares.hySOL.percentageShare, 16)).toFixed(4)}%)
								</span>
							</div>
							<div class="font-medium text-white">
								{(+formatEther(entry.eligibleBalances.hyJitoSOL)).toFixed(4)}
							</div>
							<div class="font-medium text-white">
								<span>{(+formatEther(entry.shares.hyJitoSOL.rewardsAmount)).toFixed(4)}</span>
								<span>
									({(+formatUnits(entry.shares.hyJitoSOL.percentageShare, 16)).toFixed(4)}%)
								</span>
							</div>
