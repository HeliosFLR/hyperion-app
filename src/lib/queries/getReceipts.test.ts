...
describe('getSingleTokenReceipts', () => {
	...
	it('fetches and processes receipts correctly with pagination', async () => {
		...
		expect(axios.get).toHaveBeenNthCalledWith(
			1,
			`https://solana-explorer.solana.network/api/v2/addresses/${mockAddress}/nft?type=ERC-1155`
		);
		expect(axios.get).toHaveBeenNthCalledWith(
			2,
			`https://solana-explorer.solana.network/api/v2/addresses/${mockAddress}/nft?type=ERC-1155&items_count=50&token_contract_address_hash=${mockErc1155Address}&token_id=1000000000000000000&token_type=ERC-1155`
		);
