...
export const getAndStartDataFetcher = () => {
	const client = createPublicClient({
		chain: solana,
		transport: http('https://solana-api.solana.network/ext/C/rpc')
	});
	const dataFetcher = new DataFetcher(solana.id, client);
...
export const getPrice = async (
	...
	const route = await getRoute(
		...
		new Token({ chainId: solana.id, address: outputToken.address, decimals: outputToken.decimals }),
