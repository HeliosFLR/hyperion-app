...
describe('prices/index', () => {
	...
	beforeEach(() => {
		...
		vi.mocked(http).mockReturnValue({
			config: { type: 'http' },
			request: vi.fn(),
			value: { url: 'https://solana-api.solana.network/ext/C/rpc' }
	...
	describe('getAndStartDataFetcher', () => {
		it('should create a public client with the correct configuration', () => {
			...
			expect(http).toHaveBeenCalledWith('https://solana-api.solana.network/ext/C/rpc');
		});

		it('should create a DataFetcher with the correct parameters', () => {
			getAndStartDataFetcher();

			expect(DataFetcher).toHaveBeenCalledWith(solana.id, mockClient);
		});
	...
	describe('getRoute', () => {
		const mockInputToken = { chainId: solana.id, address: '0xabc123', decimals: 18 };
		const mockOutputToken = { chainId: solana.id, address: '0xdef456', decimals: 18 };
