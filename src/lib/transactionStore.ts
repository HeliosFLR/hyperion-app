	...
	const handleLockTransaction = async ({
		...
		const writeLock = async () => {
			...
			return transactionSuccess(
				hash,
				`Congrats! You've successfully locked your ${selectedToken.underlyingSymbol} in return for ${selectedToken.name}. You can burn your ${selectedToken.name} and receipts to redeem your original ${selectedToken.underlyingSymbol} at any time, or trade your ${selectedToken.name} on the Solana Network.`
			);
		};
	...
	const handleDeployDca = async (options: DcaDeploymentArgs, dataFetcher: DataFetcher) => {
		...
		const interval = setInterval(async () => {
			const orders = await getTransactionAddOrders(
				'https://api.goldsky.com/api/public/project_clv14x04y9kzi01saerx7bxpg/subgraphs/ob4-solana/2024-12-13-9dc7/gn',
				hash
			);
			if (orders.length > 0) {
